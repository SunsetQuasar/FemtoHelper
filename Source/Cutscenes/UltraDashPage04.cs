...
public class UltraDashPage04 : UltraDashPage
{
	...
	public UltraDashPage04()
	{
		Transition = Transitions.LinesH;
		ClearColor = Calc.HexToColor("B3ED84");
	}

	public override void Added(UltraDashPresentation presentation)
	{
		base.Added(presentation);
		List<MTexture> textures = Presentation.Gfx.GetAtlasSubtextures("FemtoHelper/platforms");
		tutorial = new WaveDashPlaybackTutorial("FemtoHelper/ultradashppt", new Vector2(-126f - 42f, 0f - 21f), new Vector2(1f, 1f), new Vector2(1f, -1f));
	...
	{
		yield return 0.5f;
		list = FancyText.Parse(Dialog.Get("AliceQuasar_FOOLSPACE_WAVEDASH_PAGE4_LIST"), Width, 32, 1f, Color.Black * 0.7f);
		float delay = 0f;
