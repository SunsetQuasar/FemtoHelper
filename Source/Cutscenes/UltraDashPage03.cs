...
public class UltraDashPage03 : UltraDashPage
{
	...
	public UltraDashPage03()
	{
		Transition = Transitions.Rotate3D;
		ClearColor = Calc.HexToColor("E49EC1");
		title = Dialog.Clean("AliceQuasar_FOOLSPACE_WAVEDASH_PAGE3_TITLE");
		titleDisplayed = "";
	}
	...
	public override void Added(UltraDashPresentation presentation)
	{
		...
		clipArt = presentation.Gfx["FemtoHelper/movesetUltra"];
	}
	...
	public override IEnumerator Routine()
	{
		...
		Audio.Play("event:/FemtoHelper/ppt_ultradash_whoosh");
		...
		yield return 0.25f;
		infoText = FancyText.Parse(Dialog.Get("AliceQuasar_FOOLSPACE_WAVEDASH_PAGE3_INFO"), Width - 240, 32, 1f, Color.Black * 0.7f);
		yield return PressButton();
		Audio.Play("event:/new_content/game/10_farewell/ppt_its_easy");
		easyText = new ReverseAreaCompleteTitle(new Vector2((float)Width / 2f, Height - 150), Dialog.Clean("AliceQuasar_FOOLSPACE_WAVEDASH_PAGE3_EASY"), 2f, rainbow: true);
		yield return 1f;
		complementText = true;
	}
	...
	public override void Render()
	{
		...
		if (clipArtEase > 0f)
		{
			Vector2 scale = Vector2.One * (-1f - (1f - clipArtEase) * 1f) * -0.5f;
		...
		if (complementText)
		{
			ActiveFont.Draw(Dialog.Clean("AliceQuasar_FOOLSPACE_WAVEDASH_PAGE3_EASY_COMPLEMENT"), new Vector2(20f, (float)base.Height - 40f), new Vector2(0f, 0f), new Vector2(0.5f, 0.5f), Color.Black * 0.4f);
		}
	}
}
